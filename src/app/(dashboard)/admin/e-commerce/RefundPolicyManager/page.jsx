// src/components/admin/RefundPolicyManager.jsx
"use client";
...
export default function RefundPolicyManager() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const res = await fetch(`/api/refund-policy/${editingSection._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          section: formData.section,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`/api/refund-policy/${id}`, {
        method: 'DELETE'
      });
        ...
        <form onSubmit={editingSection ? handleUpdate : handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              required
              placeholder="e.g., Order Cancellation, Refund Eligibility"
